...
public slots:
    ...
    QString copyString( const QString& s ) 
	{ 
		return s; 
	}
    QVariantMap copyVariantMap( const QVariantMap& vm ) 
	{ 
		return vm; 
	}
    QVariantList copyVariantList( const QVariantList& vl ) 
	{ 
		return vl; 
	}
    QObject* createObject() { 
        VectorObject* mo = new VectorObject; 
        mo->setObjectName( "New Object" );
        return mo; //WARNING: NOT DESTROYED WHEN GARBAGE COLLECTED 
                   //SINCE DEFAULT IS 'QOBJ_NO_DELETE'
    }
    QList< float > copyFloatList( const QList< float >& l ) 
	{ 
		return l; 
	}
    ...
    QList< short > copyShortList( const QList< short >& l ) 
	{ 
		return l; 
	}
    QVector< short > copyShortVector( const QVector< short >& v ) 
	{ 
		return v; 
	}
signals:
